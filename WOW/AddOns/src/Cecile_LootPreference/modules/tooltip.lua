...
function C_LP.TooltipHookFunc(tooltip, ...)
	...
	if (link) then
		...
		results = wipe(results)
 
		for k,v in pairs(C_LP.guilditems) do
 
			if v.items[itemID] and (not (v.items[itemID]=="")) then
				tinsert(results,{player=k,class=v.class,priority=v.items[itemID]})
			end
		end
		...
		if not (total==0) then
			...
			for i = 1, total do
				local classcolor = RAID_CLASS_COLORS[results[i].class]
 
				priority_text = L[results[i].priority]
 
				if C_LP.epgp then					
					if results[i].epgp_pr > 9999 then
						priority_text = ("%s (PR:%d)"):format(priority_text,math.floor(results[i].epgp_pr))
					else
						priority_text = ("%s (PR:%.4g)"):format(priority_text,results[i].epgp_pr)
					end					
				end
